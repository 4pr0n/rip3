...
class AES(object):
	...
	def KeyExpansion(key):
			...
			for i in range(Nk, Nb*(Nr+1)):
					...
					if i%Nk == 0:
							...
							for t in range(4): temp[t] ^= AES.Rcon[i/Nk][t]
	...
	@staticmethod
	def SubWord(w):
			for i in range(4): w[i] = AES.Sbox[w[i]]
			return w
...
#    password  = password.encode("utf-8")
			...
			for i in range(4): counterBlock[i] = AES.urs(nonceSec, i*8) & 0xff
			...
			for b in range(blockCount):
					for c in range(4): counterBlock[15-c] = AES.urs(b, c*8) & 0xff
					for c in range(4): counterBlock[15-c-4] = AES.urs(b/0x100000000, c*8)
...
#    password = password.encode("utf-8")
			...
			for b in range(nBlocks):
					for c in range(4): 
							counterBlock[15-c] = AES.urs(b, c*8) & 0xff
					for c in range(4): 
							counterBlock[15-c-4] = AES.urs( int( float(b+1)/0x100000000-1 ), c*8) & 0xff
